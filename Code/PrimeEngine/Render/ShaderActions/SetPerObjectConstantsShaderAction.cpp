...
#	elif APIABSTRACTION_D3D9
    ...
    HRESULT hr = pDevice->SetVertexShaderConstantF(81, (const float *)(&m_data), dataTouse / (4*sizeof(float)));
	assert(SUCCEEDED(hr));
	hr = pDevice->SetPixelShaderConstantF(81, (const float *)(&m_data), dataTouse / (4*sizeof(float)));
	assert(SUCCEEDED(hr));
