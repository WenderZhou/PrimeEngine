	...
	if (!haveObject)
	{
		...
		else
		{
			if (pRealEvent->hasCustomOrientation)
			{
				...
				// add physics component for mesh
				pSN->m_hCollider = m_pContext->getPhysicsManager()->addCollider();
				Collider* pCollider = pSN->m_hCollider.getObject<Collider>();

				pCollider->m_type = Collider::STATIC;

				Mesh* pMesh = pMeshInstance->m_hAsset.getObject<Mesh>();
				//Vector3* vertices = pMesh->m_boundingVolume.vertices;
				Vector3 vertices[8];
				for (int i = 0; i < 8; i++)
					vertices[i] = pSN->m_base * pMesh->m_boundingVolume.vertices[i];

				for (int i = 0; i < 8; i++)
					pCollider->vertices[i] = pSN->m_base * pMesh->m_boundingVolume.vertices[i];

				pCollider->normals[0] = (vertices[1] - vertices[0]).crossProduct(vertices[2] - vertices[0]);
				pCollider->normals[1] = (vertices[5] - vertices[7]).crossProduct(vertices[6] - vertices[7]);
				pCollider->normals[2] = (vertices[2] - vertices[0]).crossProduct(vertices[4] - vertices[0]);
				pCollider->normals[3] = (vertices[3] - vertices[7]).crossProduct(vertices[5] - vertices[7]);
				pCollider->normals[4] = (vertices[4] - vertices[0]).crossProduct(vertices[1] - vertices[0]);
				pCollider->normals[5] = (vertices[6] - vertices[7]).crossProduct(vertices[3] - vertices[7]);

				for (int i = 0; i < 6; i++)
					pCollider->normals[i].normalize();

				pCollider->d[0] = -pCollider->normals[0] * vertices[0];
				pCollider->d[1] = -pCollider->normals[1] * vertices[7];
				pCollider->d[2] = -pCollider->normals[2] * vertices[0];
				pCollider->d[3] = -pCollider->normals[3] * vertices[7];
				pCollider->d[4] = -pCollider->normals[4] * vertices[0];
				pCollider->d[5] = -pCollider->normals[5] * vertices[7];
			}
			else
			{
