...
int ClientGame::runGameFrame()
{
	...
	// Create Physics End Event
	{
		Handle hphysicsEndEvt("EVENT", sizeof(Event_PHYSICS_END));
		Event_PHYSICS_END *physicsEndEvt = new(hphysicsEndEvt) Event_PHYSICS_END;

		physicsEndEvt->m_frameTime = m_frameTime;
		Events::EventQueueManager::Instance()->add(hphysicsEndEvt, Events::QT_GENERAL);
	}
    ...
    while (!gqh.getObject<Events::EventQueue>()->empty())
    {
        ...
        if (Event_UPDATE::GetClassId() == pGeneralEvt->getClassId())
        {
			...
			// Update particles
			ParticleRenderer::Instance()->handleEvent(pGeneralEvt);
        }
        else if (Event_CALCULATE_TRANSFORMATIONS::GetClassId() == pGeneralEvt->getClassId())
        {
            ...
            // Push DRAW event into message queue because camera has updated transformations
            {
				...
				drawEvt->m_frustum = pcam->m_frustum;
			...
			|| Event_GATHER_DRAWCALLS_Z_ONLY::GetClassId() == pGeneralEvt->getClassId())
        {
			...
			if (!zOnly)
			{
				...
				ParticleRenderer::Instance()->postPreDraw(m_pContext->m_gameThreadThreadOwnershipMask);
        ...
        {
			// physics kick off
			Event_PHYSICS_START *pRealEvent = (Event_PHYSICS_START*)(pGeneralEvt);
			m_pContext->getPhysicsManager()->handleEvent(pRealEvent);
        }
        else
        {
            // unknown event
