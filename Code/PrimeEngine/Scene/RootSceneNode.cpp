...
void RootSceneNode::do_GATHER_DRAWCALLS(Events::Event *pEvt)
{
	...
	if (setGlobalValues)
	{
		...
		// set wind source
		/*psvPerObjectGroup->m_data.gWindSources[0].pos = pDrawEvent ? pDrawEvent->m_eyePos : pZOnlyDrawEvent->m_eyePos;
		psvPerObjectGroup->m_data.gWindSources[0].pad0 = pDrawEvent ? fabsf(sinf(pDrawEvent->m_gameTime)) : 0.0f;// 1.0f;

		int extraWindSourceCount = 0;

		Handle* cIter = m_components.getFirstPtr();
		for (PrimitiveTypes::UInt32 i = 0; i < m_components.m_size && extraWindSourceCount < 2; i++)
		{
			Component* pComponent = cIter->getObject<Component>();
			if (pComponent->isInstanceOf(SceneNode::GetClassId()))
			{
				SceneNode* pSN = (SceneNode*)pComponent;
				if (pSN->m_hCollider.isValid())
				{
					Collider* pCollider = pSN->m_hCollider.getObject<Collider>();
					if (pCollider->m_type == Collider::DYNAMIC)
					{
						extraWindSourceCount++;
						psvPerObjectGroup->m_data.gWindSources[extraWindSourceCount].pos = pSN->m_base.getPos();
						psvPerObjectGroup->m_data.gWindSources[extraWindSourceCount].pad0 = pDrawEvent ? fabsf(sinf(pDrawEvent->m_gameTime)) : 0.0f;
					}
				}
			}
			++cIter;
		...
		for (int i = extraWindSourceCount + 1; i < 3; i++)
		{
			psvPerObjectGroup->m_data.gWindSources[i].pad0 = 0.0f;
		}*/
	}
}
}; // namespace Components
