...
void SceneNode::addDefaultComponents()
{
	...
	PE_REGISTER_EVENT_HANDLER(Events::Event_PHYSICS_END, SceneNode::do_PHYSICS_END);
}
...
void SceneNode::do_PHYSICS_END(Events::Event* pEvt)
{
	Events::Event_PHYSICS_END* physicsEndEvt = (Events::Event_PHYSICS_END*)(pEvt);
	if (m_hCollider.isValid())
	{
		Collider* pCollider = m_hCollider.getObject<Collider>();
		if (pCollider && pCollider->m_type == Collider::DYNAMIC)
		{
			Vector3 position = pCollider->position;
			position.m_y -= pCollider->radius;
			//pCollider->decendingSpeed = (m_base.m[1][3] - position.m_y) / physicsEndEvt->m_frameTime;

			m_base.setPos(position);
		}
	}
}
