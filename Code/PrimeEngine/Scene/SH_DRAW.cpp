...
void SingleHandler_DRAW::do_GATHER_DRAWCALLS(Events::Event *pEvt)
{
	...
	//			for (int i = 0; i < 8; i++)
	//			{
	//				/*Vector3 vertexPerspective = pDrawEvent->m_projectionViewTransform * base * pMeshCaller->m_boundingVolume.vertex[i];
	//				if (vertexPerspective.m_x > -1.0 && vertexPerspective.m_x < 1.0 &&
	//					vertexPerspective.m_y > -1.0 && vertexPerspective.m_y < 1.0 &&
	//					vertexPerspective.m_z > 0 && vertexPerspective.m_z < 1.0)*/
	//				if(pDrawEvent->m_frustum.inside(base * pMeshCaller->m_boundingVolume.vertex[i]))
	//				{
