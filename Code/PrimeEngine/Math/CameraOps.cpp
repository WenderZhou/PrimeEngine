...
namespace CameraOps
{
		...
		PrimitiveTypes::Float32 nearClip, PrimitiveTypes::Float32 farClip)
	{
		...
		// I believe following is wrong calculation
		...
		float sine = sinf(verticalFov / 2);
		/* Should be non-zero to avoid division by zero. */
		assert(deltaZ);
		assert(sine);
		assert(aspectRatio);
		float cotangent = cosf(verticalFov / 2) / sine;

		res2.m[0][0] = cotangent / aspectRatio;
		res2.m[0][1] = 0.0f;
		res2.m[0][2] = 0.0f;
		res2.m[0][3] = 0.0f;

		res2.m[1][0] = 0.0f;
		res2.m[1][1] = cotangent;
		res2.m[1][2] = 0.0f;
		res2.m[1][3] = 0.0f;

		res2.m[2][0] = 0.0f;
		res2.m[2][1] = 0.0f;
		res2.m[2][2] = farClip / deltaZ;
		res2.m[2][3] = -1  * nearClip * farClip / deltaZ;
