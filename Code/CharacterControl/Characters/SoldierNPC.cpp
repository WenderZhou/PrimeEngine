...
SoldierNPC::SoldierNPC(PE::GameContext &context, PE::MemoryArena arena, PE::Handle hMyself, Event_CreateSoldierNPC *pEvt) : Component(context, arena, hMyself)
{
	...
	// add physics component for soldier
	pMainSN->m_hCollider = m_pContext->getPhysicsManager()->addCollider();
	Collider* pCollider = pMainSN->m_hCollider.getObject<Collider>();

	pCollider->m_type = Collider::DYNAMIC;
	pCollider->radius = 0.2f;

	pCollider->gravity = 10.0f;
	pCollider->decendingSpeed = 0.0f;
	...
	pSoldierMovementSM->m_shootWhileStanding = pEvt->m_shootWhileStanding;
