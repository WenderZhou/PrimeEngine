...
void SoldierNPCMovementSM::do_UPDATE(PE::Events::Event *pEvt)
{
	if (m_state == WALKING_TO_TARGET || m_state == RUNNING_TO_TARGET)
	{
		...
		if (pSN)
		{
			...
			if (dsqr > 0.01f)
			{
				...
				PE::Handle hCollider = pSN->m_hCollider;
				Collider* pCollider = hCollider.getObject<Collider>();
				pCollider->position = curPos + dir * dist;
				pCollider->position.m_y += pCollider->radius;
				pCollider->position.m_y -= pCollider->decendingSpeed * pRealEvt->m_frameTime;
				pCollider->decendingSpeed += pCollider->gravity * pRealEvt->m_frameTime;
			}
	...
	else if (m_state == STANDING && m_shootWhileStanding) // keep turning to the target
	{
		...
		if (pGameObjectManagerAddon)
		{
			...
			SceneNode* pSNSol = getParentsSceneNode();
			if (pSNSol)
			{
				PE::Handle hTarget = pTarget->getHandle();
				SceneNode* pSNTarget = hTarget.getObject<Component>()->getFirstComponent<SceneNode>();

				Vector3 curPos = pSNSol->m_base.getPos();
				Vector3 targetPos = pSNTarget->m_base.getPos();

				Vector3 dir = targetPos - curPos;
				dir.normalize();

				pSNSol->m_base.turnInDirection(dir, 3.1415f);
			}
		}
	}
}
