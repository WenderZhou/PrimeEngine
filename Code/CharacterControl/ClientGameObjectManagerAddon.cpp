...
void ClientGameObjectManagerAddon::addDefaultComponents()
{
	...
	PE_REGISTER_EVENT_HANDLER(Event_CREATE_TARGET, ClientGameObjectManagerAddon::do_CREATE_TARGET);
...
void ClientGameObjectManagerAddon::do_CREATE_TARGET(PE::Events::Event* pEvt)
{
	PEINFO("GameObjectManagerAddon::do_CREATE_TARGET()\n");

	assert(pEvt->isInstanceOf<Event_CREATE_TARGET>());

	Event_CREATE_TARGET* pTrueEvent = (Event_CREATE_TARGET*)(pEvt);
