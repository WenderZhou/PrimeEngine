...
int Event_CREATE_WAYPOINT::l_Construct(lua_State* luaVM)
{
	...
	const char* nextWayPointNames = lua_tostring(luaVM, -numArgs--);
	...
	// set next wayPoint's name (atmost 3)
	int nextWayPointCnt = 0;
	char tempNextWayPointNames[100];
	strcpy(tempNextWayPointNames, nextWayPointNames);
	char* nextWayPointName = strtok(tempNextWayPointNames, ",");
	while (nextWayPointName != NULL && nextWayPointCnt < 3)
	{
		StringOps::writeToString(nextWayPointName, pEvt->m_nextWaypointName[nextWayPointCnt], 32);
		nextWayPointCnt++;
		nextWayPointName = strtok(NULL, ",");
	}
	pEvt->m_nextWayPointCnt = nextWayPointCnt;
...
: Component(context, arena, hMyself)
{
	...
	m_nextWayPointCnt = pEvt->m_nextWayPointCnt;
	for(int i = 0; i < m_nextWayPointCnt; i++)
		StringOps::writeToString(pEvt->m_nextWaypointName[i], m_nextWayPointName[i], 32);
